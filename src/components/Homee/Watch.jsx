  ...
  useEffect(()=>{
      axios.get('https://api.themoviedb.org/3/movie/popular', {
        ...
        .then( (response)=> {
          ...
          return setData(response.data.results.slice(7,12))
      })
        .catch( (error)=> {
      ...
      <img className='bg-gray-800 p-1 rounded-sm' src={left} alt="" />
       ...
       <img className='bg-gray-800 p-1 rounded-sm' src={right} alt="" />
        ...
        <Link to={'/movies'} key={movie.id} className="p-8 bg-gray-900 rounded-xl shadow ">
           ...
           <span className="text-white flex items-center text-xs "><img  src={time} alt="" />2h 10min</span>
            <div>
            <img className="w-[100px]" src={star} alt="" />
            </div>
           </div>
        </Link>
        ))}
